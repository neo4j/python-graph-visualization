...
We start by instantiating the link:{api-uri}/node/[Nodes] and link:{api-uri}/relationship[Relationships] we want in our graph.
...
Lastly we create a link:{api-uri}/visualization-graph/[VisualizationGraph] object with the nodes and relationships we created, and call its `render` method to display the graph.
...
[TIP]
====
See the link:{tutorials-uri}/getting-started/[Jupyter notebook] to interact with the graph.
====
...
* Browse the link:{tutorials-uri}[notebook examples] for more examples and use cases
...
* Read about our link:{api-uri}/[API reference]
