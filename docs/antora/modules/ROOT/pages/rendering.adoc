...
This will return an `IPython.display.HTML` object that will be displayed in an environment that supports HTML rendering, such as
Jupyter notebooks or Streamlit applications.

All parameters of the `render` method are optional, and the full list of parameters is listed in the API
...
A tooltip showing IDs and properties will be shown when mouse hovering over a node or relationship.
...
== The `render_widget` method

The `render_widget` method returns a `GraphWidget` (anywidget) that provides two-way data sync between Python and JavaScript.
This works in JupyterLab, Notebook 7, VS Code, and Colab.
...
The object returned by the `render` method is an `IPython.display.HTML` object.
...
The `to_html` method is an alias for `render` with the same parameters.