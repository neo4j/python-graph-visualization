...
The interactive widget can save the current visualization as a PNG or SVG image file from a notebook cell:
...
The format is inferred from the file extension (`.png` or `.svg`) and defaults to SVG when the file has no recognized extension; an explicit `format="png"`/`format="svg"` argument must match the extension.
