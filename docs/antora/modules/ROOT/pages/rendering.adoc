...
`neo4j_viz.streamlit.render_widget`. It renders the same interactive widget as
`render_widget` does in a notebook and syncs the `selected` and `options` state
back into the returned `GraphWidget` on each rerun.
...
widget = render_widget(VG.render_widget(), key="my-graph")

selection = widget.selected
st.write("Selected node IDs:", selection.nodeIds)
----

`render_widget` takes a `GraphWidget` (as returned by
...
[NOTE]
====
Streamlit does not support the Jupyter/anywidget comm protocol, so a
`GraphWidget` cannot be embedded directly (nor via the unmaintained
`streamlit-anywidget` bridge, which ignores the widget's custom traitlets
serializers). `render_widget` bridges the widget as a Streamlit
`Components v2` component instead. The graph follows the Streamlit light/dark
theme automatically; pass `theme="light"` or `theme="dark"` to
`VisualizationGraph.render_widget()` to pin it.
====
