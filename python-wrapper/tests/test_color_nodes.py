...
def test_color_nodes_default_override() -> None:
    """Test that the default value of override is False (existing colors are preserved by default)."""
    nodes = [
    ...
    # Call without specifying override - should use default (False) and preserve existing colors
    VG.color_nodes(field="caption", colors={"Person": "#000000", "Product": "#00FF00"})

    assert VG.nodes[0].color == Color("#FF0000")  # Preserved
    assert VG.nodes[1].color == Color("#FF0000")  # Preserved
    assert VG.nodes[2].color == Color("#00ff00")  # Was None, so gets new color
